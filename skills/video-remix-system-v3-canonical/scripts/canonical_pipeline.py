...
def validate_source_performance_contract(generation_gate, shot, prompt_path, enabled):
    """Require observable source performance evidence for new V3 projects."""
    if not enabled:
        return
...
def compile_project(source_path, out_dir):
    ...
    if image_provider.get("type") != "seedream":
        err("当前V3图像层仅支持seedream provider")
    ...
    # V3 semantic-role-performance gate is a hard input, not an optional report.
    ...
    receipt={
        "system":"video-remix-system-v3-canonical","version":"3.0",
    ...
    final_manifest={
        ...
        "video_generation":{"status":video_status,"reason":"V3 compile produces manifests only; video submission requires an explicit configured backend."}
    }
