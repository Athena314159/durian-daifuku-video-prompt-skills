#!/usr/bin/env python3
"""Export a V2 Canonical build into an auditable DOCX.
...
def export_docx(build_dir: Path, output_docx: Path, alignment_path: Path | None = None) -> Path:
    ...
    document = Document()
    document.add_heading(f"V2 Canonical交付 · {metadata['final'].get('project_id', '')}", level=0)
    document.add_paragraph("本文件由Canonical编译产物排版生成；每个镜头的口播、Prompt和QA图均来自同一组已校验manifest。")
    alignment: dict[str, Any] = {
        "schema": "v2-canonical-docx-alignment-v1",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Export V2 Canonical build to DOCX")
