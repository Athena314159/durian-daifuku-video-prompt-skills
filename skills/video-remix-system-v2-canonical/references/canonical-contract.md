...
源视频首次接入先写入 `source_video_sha256`、`source_transcript_sha256` 与 `script_version`。用户交回完整新版口播后，`scripts/source_handoff.py diff` 生成改稿影响清单；line ID 不完整时阻断，正文变化会使语义门、Prompt、图片清单和DOCX进入 stale，必须重新编译。只修改 `script_shot_map.json` 的正文不构成有效改稿。
...
- `revision_impact.json`：改稿的 line ID 差异、受影响镜头和失效产物。
...
图像任务还必须保留 `prompt_file`、Prompt正文哈希、身份参考、产品形态参考、包装可见面参考、几何引导图及各自SHA-256。源帧不能作为批准图；批准图没有实际文件和哈希时只能保持 `awaiting_generation`。Seedream干跑必须先构造完整请求摘要，再允许联网。
...
DOCX只能由 `scripts/export_docx_from_build.py` 消费同一编译目录的五份manifest生成。每个 `selected_shots` 必须同时具有当前口播、当前Prompt和 `user_approved`/`generated_qa_pass` 图片及哈希；缺图、旧哈希、包装/动作状态错位或 `awaiting_generation` 直接阻断，不生成部分完成文档。历史项目导出脚本不能作为V2事实源。
