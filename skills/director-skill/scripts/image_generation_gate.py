...
def assert_current_project(project: dict[str, Any], *, project_dir: Path | None = None, require_image_tier: bool = True) -> dict[str, Any]:
...
def packaging_reference_binding(project_dir: Path, project: dict[str, Any], product: dict[str, Any], shot: dict[str, Any]) -> list[dict[str, str]]:
    ...
    for level in levels:
        ...
        for face in faces:
            ...
            if level != "shipping_carton" and asset_id != expected:
                raise GateError(f"DAIFUKU_PACKAGING_REFERENCE_MISSING: {level}/{face} must bind {expected!r}")
            if level == "shipping_carton" and not asset.get("sha256"):
                raise GateError("DAIFUKU_PACKAGING_REFERENCE_NOT_USER_APPROVED: shipping-carton master must be project-specific and hash-bound")
            ...
            records.append({"packaging_level": str(level), "visible_face": str(face), "asset_id": str(asset_id), "path": relative(project_dir, path), "sha256": sha256_file(path)})
    return records
...
def authorize(args: argparse.Namespace) -> dict[str, Any]:
    ...
    release = assert_current_project(project, project_dir=project_dir)
    ...
    package_bindings = packaging_reference_binding(project_dir, project, product, shot)
...
def record_result(args: argparse.Namespace) -> dict[str, Any]:
    ...
    packaging_visible = packaging_lock.get("visible") is True if packaging_lock is not None else state.get("packaging") not in (None, False, "none", "hidden")
