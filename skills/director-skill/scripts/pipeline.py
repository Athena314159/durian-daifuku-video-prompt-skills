...
def validate_project(project_dir: Path, bundle: Dict[str, Dict[str, Any]], issues: List[Dict[str, Any]]) -> None:
    ...
    execution_redirect_path = project_dir / "planning" / "execution_redirect.json"
    if execution_redirect_path.is_file():
        execution_redirect = load_json(execution_redirect_path)
        if execution_redirect.get("status") == "redirected":
            add_issue(
                issues,
                "ERROR",
                "PROJECT_EXECUTION_REDIRECTED",
                "planning/execution_redirect.json",
                f"This historical project is rollback-only. Continue from {execution_redirect.get('target_project') or 'the recorded migrated target'}.",
            )
    ...
    validate_durian_daifuku_v2_sequence(product, shot_items, issues)
...
def validate_durian_daifuku_v2_shot(
    ...
    *,
    ...
    shape_contract = product.get("shape_contract") or {}
    shape_lock = state_data.get("shape_lock") or {}
    if shape_lock:
        if shape_lock.get("geometry_identity_id") != shape_contract.get("geometry_identity_id"):
            add_issue(issues, "ERROR", "DAIFUKU_SHAPE_IDENTITY_INVALID", f"{base}.product_state.shape_lock.geometry_identity_id", "Bind the exact approved geometry identity across packaged, plated and held contexts.")
        if shape_lock.get("silhouette_family") != shape_contract.get("silhouette_family"):
            add_issue(issues, "ERROR", "DAIFUKU_SILHOUETTE_FAMILY_INVALID", f"{base}.product_state.shape_lock.silhouette_family", "Use the rounded_slightly_oblate silhouette family for every whole or reconstructable daifuku.")
        if shape_lock.get("cross_context_identity_required") is not True:
            add_issue(issues, "ERROR", "DAIFUKU_CROSS_CONTEXT_SHAPE_UNLOCKED", f"{base}.product_state.shape_lock.cross_context_identity_required", "The packaged, plated and held products must share one geometry identity.")
        if shape_lock.get("container_shape_inheritance") is not False:
            add_issue(issues, "ERROR", "DAIFUKU_CONTAINER_SHAPE_INHERITANCE", f"{base}.product_state.shape_lock.container_shape_inheritance", "A square box or tray cell may support and occlude the food but must never make the daifuku square.")
        if float(shape_lock.get("maximum_straight_edge_fraction") or 1) > float(shape_contract.get("maximum_straight_edge_fraction") or 0.18):
            add_issue(issues, "ERROR", "DAIFUKU_STRAIGHT_EDGE_LIMIT_INVALID", f"{base}.product_state.shape_lock.maximum_straight_edge_fraction", "Lock substantially visible whole-product contours to short incidental straight segments only, never four box-like sides.")
        if shape_lock.get("maximum_right_angle_corner_count") != 0:
            add_issue(issues, "ERROR", "DAIFUKU_RIGHT_ANGLE_CORNER_INVALID", f"{base}.product_state.shape_lock.maximum_right_angle_corner_count", "A daifuku silhouette has no right-angle corners.")

    integrity_lock = state_data.get("integrity_lock") or {}
    integrity_contract = product.get("integrity_contract") or {}
    if integrity_lock:
        forbidden_flags = ("large_excavated_crater", "peeled_top_cap", "scooped_hollow", "open_basin", "hand_torn_hole_as_bite")
        if any(integrity_lock.get(field) is not False for field in forbidden_flags):
            add_issue(
                issues,
                "ERROR",
                "DAIFUKU_STATE_TOPOLOGY_INVALID",
                f"{base}.product_state.integrity_lock",
                "Reject peeled caps, excavated craters, scooped hollows, open basins and hand-torn holes; none is a whole product or a valid bite.",
            )
        if integrity_lock.get("declared_state") != state_id:
            add_issue(issues, "ERROR", "DAIFUKU_INTEGRITY_STATE_MISMATCH", f"{base}.product_state.integrity_lock.declared_state", "Integrity topology must bind the exact terminal product state.")
        if state_id in set(integrity_contract.get("whole_states") or []):
            if integrity_lock.get("opening_origin") != "none" or integrity_lock.get("filling_visibility") != "none" or integrity_lock.get("whole_shell_closed") is not True:
                add_issue(issues, "ERROR", "DAIFUKU_WHOLE_SHELL_BREACHED", f"{base}.product_state.integrity_lock", "Whole, held, plated, pressed and closed-stretch states require a fully closed shell with no filling exposure.")
        if state_id == "bitten":
            try:
                removed_ratio = float(integrity_lock.get("removed_mass_ratio"))
            except (TypeError, ValueError):
                removed_ratio = -1.0
            valid_bite = (
                integrity_lock.get("opening_origin") == "teeth_contact"
                and integrity_lock.get("filling_visibility") == "bite_notch_only"
                and integrity_lock.get("single_localized_concave_notch") is True
                and has_text(integrity_lock.get("tooth_compression_evidence"))
            ...
            if not valid_bite:
                add_issue(
                    issues,
                    "ERROR",
                    "DAIFUKU_FAKE_BITE_TOPOLOGY",
                    f"{base}.product_state.integrity_lock",
                    "A bitten state needs one localized concave notch caused by teeth, observable tooth-compression evidence and 15–30% removed mass; a large open crater is never a bite.",
                )
            if shot.get("visual_type") == "person_eating":
                orientation_valid = (
                    integrity_lock.get("opening_direction") == "toward_mouth_and_person"
                    and integrity_lock.get("mouth_contacts_opening_side") is True
                    and integrity_lock.get("camera_facing_opening") is False
                    and has_text(integrity_lock.get("orientation_evidence"))
                )
                if not orientation_valid:
                    add_issue(
                        issues,
                        "ERROR",
                        "DAIFUKU_BITE_ORIENTATION_INVALID",
                        f"{base}.product_state.integrity_lock",
                        "While the person is eating or the product remains at the mouth, the bite opening must face the mouth/person and the mouth must contact that opening side; it cannot face the camera or audience while the intact back touches the mouth.",
                    )

    instance_lock = state_data.get("instance_lock") or {}
    instance_contract = product.get("multi_instance_contract") or {}
    if instance_lock:
        try:
            source_count = int(instance_lock.get("source_product_count"))
            target_count = int(instance_lock.get("target_product_count"))
            declared_count = int(state_data.get("count"))
        except (TypeError, ValueError):
            source_count = target_count = declared_count = -1
        instance_ids = instance_lock.get("instance_ids") if isinstance(instance_lock.get("instance_ids"), list) else []
        variants = instance_lock.get("shape_variant_ids") if isinstance(instance_lock.get("shape_variant_ids"), list) else []
        event_type = instance_lock.get("count_change_event")
        if target_count <= 0 or target_count != declared_count or len(instance_ids) != target_count or len(set(map(str, instance_ids))) != target_count:
            add_issue(issues, "ERROR", "DAIFUKU_INSTANCE_ID_COVERAGE_MISMATCH", f"{base}.product_state.instance_lock", "Target count, product_state.count and unique stable instance IDs must cover the same visible product inventory.")
        if event_type == "none" and source_count != target_count:
            add_issue(issues, "ERROR", "DAIFUKU_SOURCE_COUNT_MISMATCH", f"{base}.product_state.instance_lock", "Without a visible inventory-changing action, preserve the source product count exactly, including two or three products held at once.")
        if instance_lock.get("shared_size_class") != instance_contract.get("shared_size_class"):
            add_issue(issues, "ERROR", "DAIFUKU_MULTI_INSTANCE_SIZE_CLASS_INVALID", f"{base}.product_state.instance_lock.shared_size_class", "All instances must share the approved approximately-7-cm size class.")
        if instance_lock.get("pixel_identical_clones") is not False or len(variants) != target_count or (target_count > 1 and len(set(map(str, variants))) != target_count):
            add_issue(issues, "ERROR", "DAIFUKU_MULTI_INSTANCE_CLONED", f"{base}.product_state.instance_lock", "Equal overall size does not mean cloned shape: give each item a unique stable variant and forbid pixel-identical copies.")
        if state_id == "held" and target_count > 1 and instance_lock.get("contact_deformation") != "slight_local_flattening_with_volume_conservation":
            add_issue(issues, "ERROR", "DAIFUKU_CONTACT_DEFORMATION_INVALID", f"{base}.product_state.instance_lock.contact_deformation", "Two or three stacked/gripped daifuku need slight local flattening only at contacts while conserving total volume.")

    packaging_lock = state_data.get("packaging_lock") if isinstance(state_data.get("packaging_lock"), dict) else None
    packaging_visible = (
        packaging_lock.get("visible") is True
        if packaging_lock is not None
        else state_data.get("packaging") not in (None, False, "none", "hidden")
    )
    package_content_lock = state_data.get("package_content_lock") or {}
    if packaging_visible and not package_content_lock:
        add_issue(issues, "ERROR", "DAIFUKU_PACKAGE_CONTENT_LOCK_MISSING", f"{base}.product_state.package_content_lock", "Packaging shots must inventory and validate every product instance separately from the square container.")
    if packaging_visible and package_content_lock:
        instance_ids = package_content_lock.get("instance_ids")
        package_valid = (
            package_content_lock.get("geometry_identity_id") == shape_contract.get("geometry_identity_id")
            and package_content_lock.get("container_geometry_independent") is True
            and package_content_lock.get("tray_cell_role") == "support_and_occlusion_only"
            and package_content_lock.get("per_visible_instance_shape_qa") is True
            and package_content_lock.get("package_capacity_count") == int(shape_contract.get("package_capacity_count") or 4)
            and package_content_lock.get("accounted_product_count") == int(shape_contract.get("package_capacity_count") or 4)
            and isinstance(instance_ids, list)
            and len(instance_ids) == int(shape_contract.get("package_capacity_count") or 4)
            and len({str(value) for value in instance_ids}) == len(instance_ids)
        )
        if not package_valid:
            add_issue(issues, "ERROR", "DAIFUKU_PACKAGE_PRODUCT_GEOMETRY_INVALID", f"{base}.product_state.package_content_lock", "Use four stable instance IDs, account for the full four-piece inventory, keep container geometry independent and require per-visible-instance rounded-shape QA.")

    try:
        state_count = int(state_data.get("count") or 0)
    except (TypeError, ValueError):
        state_count = 0
    arrangement_required = packaging_visible or state_id == "plated" or state_count > 1
    arrangement_lock = state_data.get("arrangement_lock") or {}
    if arrangement_required and not arrangement_lock:
        add_issue(issues, "ERROR", "DAIFUKU_LAYOUT_LOCK_MISSING", f"{base}.product_state.arrangement_lock", "Multi-product, container and packaging shots require a stable layout and inventory lock.")
    if arrangement_lock:
        arrangement_ids = arrangement_lock.get("instance_ids") if isinstance(arrangement_lock.get("instance_ids"), list) else []
        if not has_text(arrangement_lock.get("layout_id")) or not has_text(arrangement_lock.get("container_id")) or not has_text(arrangement_lock.get("inventory_stage_id")):
            add_issue(issues, "ERROR", "DAIFUKU_LAYOUT_IDENTITY_INVALID", f"{base}.product_state.arrangement_lock", "Bind stable layout, container and inventory-stage IDs.")
        if len(arrangement_ids) != len(set(map(str, arrangement_ids))):
            add_issue(issues, "ERROR", "DAIFUKU_LAYOUT_INSTANCE_DUPLICATE", f"{base}.product_state.arrangement_lock.instance_ids", "Layout instance IDs must be unique.")
        if packaging_visible and (
            arrangement_lock.get("natural_irregularity_required") is not True
            or arrangement_lock.get("perfect_grid") is not False
            or arrangement_lock.get("equal_spacing") is not False
            or arrangement_lock.get("uniform_orientation") is not False
        ):
            add_issue(issues, "ERROR", "DAIFUKU_OVERREGULAR_ARRANGEMENT", f"{base}.product_state.arrangement_lock", "Individual packs and retail boxes must use slight offsets, varied orientations and natural partial overlaps, not a perfect grid, equal spacing or all-parallel display.")
    ...
    if packaging_visible:
        packaging_base = f"{base}.product_state.packaging_lock"
        if not packaging_lock:
            add_issue(issues, "ERROR", "DAIFUKU_PACKAGING_LOCK_MISSING", packaging_base, "Packaging-visible shots must declare exact package levels, visible faces and approved reference asset IDs.")
        else:
            package_contract = product.get("packaging_contract") if isinstance(product.get("packaging_contract"), dict) else {}
            allowed_levels = set(package_contract.get("package_levels") or [])
            declared_levels = packaging_lock.get("package_levels") if isinstance(packaging_lock.get("package_levels"), list) else []
            faces_by_level = packaging_lock.get("visible_faces_by_level") if isinstance(packaging_lock.get("visible_faces_by_level"), dict) else {}
            ids_by_level = packaging_lock.get("reference_asset_ids_by_level") if isinstance(packaging_lock.get("reference_asset_ids_by_level"), dict) else {}
            if packaging_lock.get("invented_packaging_allowed") is not False or packaging_lock.get("artwork_mode") != "approved_master_projection_only":
                add_issue(issues, "ERROR", "DAIFUKU_PACKAGING_INVENTION_FORBIDDEN", packaging_base, "Packaging artwork must be projected only from an approved master; invented or inferred branding is forbidden.")
            if not declared_levels or len(set(map(str, declared_levels))) != len(declared_levels):
                add_issue(issues, "ERROR", "DAIFUKU_PACKAGING_LEVEL_INVALID", f"{packaging_base}.package_levels", "Declare each visible packaging level exactly once.")
            for level in declared_levels:
                level_base = f"{packaging_base}.{level}"
                if level not in allowed_levels:
                    add_issue(issues, "ERROR", "DAIFUKU_PACKAGING_LEVEL_INVALID", level_base, "Use individual_pouch, retail_outer_box or shipping_carton without substituting another layer.")
                    continue
                faces = faces_by_level.get(level) if isinstance(faces_by_level.get(level), list) else []
                asset_ids = ids_by_level.get(level) if isinstance(ids_by_level.get(level), dict) else {}
                if not faces:
                    add_issue(issues, "ERROR", "DAIFUKU_PACKAGING_FACE_MISSING", level_base, "List every visible face for this packaging level.")
                for face in faces:
                    asset_id = asset_ids.get(face)
                    asset = approved_assets.get(asset_id)
                    if not asset:
                        add_issue(issues, "ERROR", "DAIFUKU_PACKAGING_REFERENCE_MISSING", f"{level_base}.{face}", "Bind an approved packaging asset for this exact visible level and face.")
                        continue
                    if asset.get("packaging_level") != level or asset.get("visible_face") != face:
                        add_issue(issues, "ERROR", "DAIFUKU_PACKAGING_LAYER_MISMATCH", f"{level_base}.{face}", "A packaging asset cannot satisfy another packaging level or visible face.")
                    if asset.get("user_approved") is not True:
                        add_issue(issues, "ERROR", "DAIFUKU_PACKAGING_REFERENCE_NOT_USER_APPROVED", f"{level_base}.{face}", "Packaging references require explicit user approval; status text alone is not approval.")
                    expected_id = (((package_contract.get("required_reference_assets_by_level_and_face") or {}).get(level) or {}).get(face))
                    if level != "shipping_carton" and asset_id != expected_id:
                        add_issue(issues, "ERROR", "DAIFUKU_PACKAGING_REFERENCE_MISSING", f"{level_base}.{face}", f"Use the exact globally approved asset {expected_id!r} for this level and face.")
                    if level == "shipping_carton" and not has_text(asset.get("sha256")):
                        add_issue(issues, "ERROR", "DAIFUKU_PACKAGING_REFERENCE_NOT_USER_APPROVED", f"{level_base}.{face}", "Shipping-carton masters must be project-specific, user-approved and hash-bound.")
                    target_relative = asset.get("target_path") or asset.get("path")
                    if target_relative not in as_list((shot.get("asset_links") or {}).get("product_references")):
                        add_issue(issues, "ERROR", "DAIFUKU_PACKAGING_REFERENCE_MISSING", f"{base}.asset_links.product_references", "The exact packaging reference file must be linked as an image input; a body-only product reference is insufficient.")
...
def validate_durian_daifuku_v2_sequence(product: Dict[str, Any], shots: List[Dict[str, Any]], issues: List[Dict[str, Any]]) -> None:
    """Validate cross-shot container inventory and layout transitions."""
    if product.get("profile_id") != "durian-daifuku-v2":
        return
    ...
    for index, shot in enumerate(shots):
        if not isinstance(shot, dict):
            continue
        state = shot.get("product_state") if isinstance(shot.get("product_state"), dict) else {}
        lock = state.get("arrangement_lock") if isinstance(state.get("arrangement_lock"), dict) else None
        if not lock:
            continue
        layout_id = str(lock.get("layout_id") or "")
        previous_id = lock.get("previous_layout_id")
        event = lock.get("event") if isinstance(lock.get("event"), dict) else {"type": "none"}
        current_ids = [str(value) for value in lock.get("instance_ids") or []]
        if previous_id:
            previous = by_layout.get(str(previous_id))
            if previous is None:
                add_issue(issues, "ERROR", "DAIFUKU_LAYOUT_PREDECESSOR_MISSING", f"shots/shot_manifest.json.shots[{index}].product_state.arrangement_lock.previous_layout_id", "A referenced previous layout must already exist earlier in the shot sequence.")
            else:
                previous_ids = [str(value) for value in previous.get("instance_ids") or []]
                if event.get("type") == "pick_up":
                    picked = str(event.get("instance_id") or "")
                    valid = (
                        picked in previous_ids
                        and event.get("picked_instance_destination") == "hand"
                        and event.get("before_count") == len(previous_ids)
                        and event.get("after_count") == len(current_ids)
                        and len(current_ids) == len(previous_ids) - 1
                        and set(current_ids) == set(previous_ids) - {picked}
                        and lock.get("relative_topology_preserved") is True
                    )
                    if not valid:
                        add_issue(issues, "ERROR", "DAIFUKU_INVENTORY_TRANSITION_INVALID", f"shots/shot_manifest.json.shots[{index}].product_state.arrangement_lock.event", "Picking one item must move that stable instance from container to hand, reduce container count by one and preserve the remaining topology.")
                elif event.get("type") == "none" and (current_ids != previous_ids or lock.get("relative_topology_preserved") is not True):
                    add_issue(issues, "ERROR", "DAIFUKU_LAYOUT_CONTINUITY_MISMATCH", f"shots/shot_manifest.json.shots[{index}].product_state.arrangement_lock", "Without a visible action, preserve instance order and relative layout across frames.")
        if layout_id:
            by_layout[layout_id] = lock
...
def compile_shot(bundle: Dict[str, Dict[str, Any]], shot: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
    ...
    if product.get("profile_id") == "durian-daifuku-v2":
        ...
        anchor = scale_lock.get("anchor") or {}
        shape_lock = product_state.get("shape_lock") or {}
        integrity_lock = product_state.get("integrity_lock") or {}
        instance_lock = product_state.get("instance_lock") or {}
        arrangement_lock = product_state.get("arrangement_lock") or {}
        package_content_lock = product_state.get("package_content_lock") or {}
        packaging_lock = product_state.get("packaging_lock") or {}
        ...
        structured_product_locks = [
            ...
            pixel_instruction,
            (
                f"形状身份={shape_lock.get('geometry_identity_id')}；完整或可重建产品始终为连续圆弧、饱满略扁圆的{shape_lock.get('silhouette_family')}；"
                f"盒内、盘中、手持共用同一几何身份={shape_lock.get('cross_context_identity_required')}；"
                f"包装盒和方格内托可以方，但只负责承托与自然遮挡，容器形状继承={shape_lock.get('container_shape_inheritance')}；"
                "不得把四条直边、直角、切角或方格压模传给大福本体"
            ),
            (
                f"完整性拓扑：状态={integrity_lock.get('declared_state')}，开口来源={integrity_lock.get('opening_origin')}，露馅范围={integrity_lock.get('filling_visibility')}；"
                "禁止顶部剥开、挖空大坑、勺挖空腔、开盆状露馅和手撕洞冒充咬口；bitten只能是牙齿接触侧一个局部内凹咬口并保留牙压证据；"
                f"人物吃时咬口方向={integrity_lock.get('opening_direction')}，嘴接触咬口侧={integrity_lock.get('mouth_contacts_opening_side')}，破口朝镜头={integrity_lock.get('camera_facing_opening')}，方向证据={integrity_lock.get('orientation_evidence')}；"
                "正在吃或产品仍在嘴边时，咬口必须朝向嘴部和人物大方向，禁止破口正对镜头、观众或背离人物，也禁止嘴接触完整背面"
            ),
            (
                f"实例数量：原片={instance_lock.get('source_product_count')}颗，目标={instance_lock.get('target_product_count')}颗，变化事件={instance_lock.get('count_change_event')}；"
                f"稳定实例={instance_lock.get('instance_ids')}，形态变体={instance_lock.get('shape_variant_ids')}，共同尺寸等级={instance_lock.get('shared_size_class')}；"
                "原片手持两颗或三颗就保留两颗或三颗；每颗整体尺寸统一但不是克隆，叠放或握持只在接触处轻微压扁并保持体积"
            ),
            (
                f"布局库存：layout={arrangement_lock.get('layout_id')}，container={arrangement_lock.get('container_id')}，stage={arrangement_lock.get('inventory_stage_id')}，实例={arrangement_lock.get('instance_ids')}，事件={arrangement_lock.get('event')}；"
                "包装、独立袋与包装盒采用轻微错位、不同朝向、自然局部遮挡，避免完美网格、等距、全平行和陈列柜式对称；盘中拿起一颗后库存减一，余下实例相对位置不无故重排"
                if arrangement_lock else ""
            ),
            (
                f"包装内容锁：容量={package_content_lock.get('package_capacity_count')}颗，已核算={package_content_lock.get('accounted_product_count')}颗，"
                f"实例ID={package_content_lock.get('instance_ids')}；逐颗轮廓QA={package_content_lock.get('per_visible_instance_shape_qa')}；"
                "逐颗检查所有实质可见的盒内大福，格边遮住一部分轮廓不等于产品变方，不能只拿一颗代表整盒"
                if package_content_lock else ""
            ),
            (
                f"包装视觉锁：层级={packaging_lock.get('package_levels')}，逐层可见面={packaging_lock.get('visible_faces_by_level')}，"
                f"逐面批准资产={packaging_lock.get('reference_asset_ids_by_level')}；"
                "独立袋、零售外盒、运输箱不可互相替代，普通大福参考不能替代包装实拍；只投射当前可见面，禁止凭空生成包装结构、品牌字样或图案"
                if packaging_lock.get("visible") is True else ""
            ),
