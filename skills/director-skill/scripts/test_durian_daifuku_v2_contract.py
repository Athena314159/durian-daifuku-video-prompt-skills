...
def main() -> int:
    with tempfile.TemporaryDirectory() as temporary:
        ...
        packaging_assets = {
            item["id"]: item
            for item in product["reference_assets"]
            if item.get("packaging_level")
        }
        assert set(packaging_assets) == {
            "DF2-PACK-POUCH-FRONT-01",
            "DF2-PACK-POUCH-SIDE-01",
            "DF2-PACK-RETAIL-FRONT-01",
        ...
        assert_code(issues, "DAIFUKU_PACKAGING_LOCK_MISSING")
        ...
        packaged["product_state"]["packaging_lock"] = {
            "visible": True,
        ...
        packaged["product_state"]["reference_roles"].append(role(retail_front))
        packaged["asset_links"]["product_references"].append(retail_front["target_path"])
        issues = []
        validate_durian_daifuku_v2_shot(project_dir, product, packaged, issues, "shot")
        assert not issues, issues

        body_only_packaging = copy.deepcopy(packaged)
        body_only_packaging["product_state"]["reference_roles"] = body_only_packaging["product_state"]["reference_roles"][:-1]
        body_only_packaging["asset_links"]["product_references"] = body_only_packaging["asset_links"]["product_references"][:-1]
        issues = []
        validate_durian_daifuku_v2_shot(project_dir, product, body_only_packaging, issues, "shot")
        assert_code(issues, "DAIFUKU_PACKAGING_REFERENCE_MISSING")

        status_only_carton = copy.deepcopy(packaged)
        carton_asset = {
            "id": "PROJECT-CARTON-FRONT",
            "path": retail_front["target_path"],
            "sha256": retail_front["sha256"],
            "role": "shipping_carton_front",
            "packaging_level": "shipping_carton",
            "visible_face": "front",
            "approved": True,
            "status": "approved_master",
        }
        status_only_carton_product = copy.deepcopy(product)
        status_only_carton_product["reference_assets"].append(carton_asset)
        status_only_carton["product_state"]["packaging_lock"].update({
            "package_levels": ["shipping_carton"],
            "visible_faces_by_level": {"shipping_carton": ["front"]},
            "reference_asset_ids_by_level": {"shipping_carton": {"front": "PROJECT-CARTON-FRONT"}},
        })
        status_only_carton["product_state"]["reference_roles"][-1] = {
            "asset_id": "PROJECT-CARTON-FRONT",
            "role": "shipping_carton_front",
            "allowed_inheritance": ["纸箱正面"],
            "forbidden_inheritance": ["凭空图案"],
        }
        issues = []
        validate_durian_daifuku_v2_shot(project_dir, status_only_carton_product, status_only_carton, issues, "shot")
        assert_code(issues, "DAIFUKU_PACKAGING_REFERENCE_NOT_USER_APPROVED")
