...
榴莲大福包装镜同时绑定 `shape_lock` 与 `package_content_lock`：容器/内托可以方，食品不可继承容器几何；盒内四颗使用稳定实例 ID，盒内、盘中、手持共用 `DF2-ROUND-7CM-001`。编译器必须写入“逐颗检查”，生成结果必须提交哈希绑定的 per-instance shape QA；任一实质可见实例出现直边、直角、切角、方格压模或跨场景形状漂移，触发 `DAIFUKU_PACKAGE_PRODUCT_GEOMETRY_INVALID`，不得批准。
