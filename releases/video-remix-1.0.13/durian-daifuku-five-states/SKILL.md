...
- 双手从完整产品开始按压、整体拉长、建立撕口并拉开：V3；按请求选择 `pressed`、`global_stretch`、`opening_window_seed`、`opening_window_established`、`pre_break` 或 `break`。用户说“正要掰开、微微露馅、往前回调”时固定使用 `opening_window_seed`。
...
1. 查看原图实际像素，识别原主体食品、人物动作、手指接触、遮挡、透视、尺度证据、破损状态与最终静态终点。生成前必须把同景深锚点实测为像素值，并计算目标大福宽高、容差和 `bbox_xywh`；只有原帧与几何引导图哈希绑定的 `authorized` 像素计划才允许调用生图工具。纯文字“约 7 厘米”或“3.5–4 指宽”不是授权。
...
6. 几何引导图只承担尺寸和位置，不是画面内容；青色轮廓、十字、标签和文字禁止进入成品。同一有效像素计划只发起一次初始生成；尺寸不合格时禁止原样盲重试，必须先修改计划中的一个可审计变量或切换确定性局部编辑。
7. 所有实际送入图像工具的 Prompt 使用中文并保存完整原文。编辑前列出唯一替换目标、锁定项、参考图角色和禁止漂移项。
   若同镜还要求换头，身份与大福替换必须从同一原始图在一次原子请求中完成；禁止先换脸再换产品、先换产品再换脸，或把单项通过的半成品回喂。任一项失败时返回原始图并重新提交完整身份、产品、尺寸与材质合同。
8. 生成结果先进入 `candidates/` 或 `diagnostic-failures/`。查看原始像素后先执行尺寸门，再完成全部适用产品门槛与场景锁定项；即使已发现其他足以判退的错误，也不得省略尺寸结论或提前结束 QA。
   包装镜额外执行逐颗形状门：盒体和方格内托可以方，但盒内每颗实质可见大福必须保持 `DF2-ROUND-7CM-001` 的连续圆弧、饱满略扁轮廓；格边只能遮挡，不能把直边、直角、切角或方格压模传给食品。盒内、盘中和手持必须是同一几何身份，不能只抽查一颗代表整盒。
   包装可见时还必须建立 `packaging_lock`：明确 `individual_pouch`、`retail_outer_box` 或 `shipping_carton`，逐个可见面绑定批准资产 ID；普通大福本体参考不能替代包装参考。独立袋必须使用 `references/individual-pouch-v1.json` 中对应正面/侧面实拍，零售盒必须使用 `references/aoqiaoxiaofang-retail-box-v1.json` 中对应面实拍。运输箱当前没有全局批准母版，缺少项目级 `approved=true`、`user_approved=true`、可见面和 SHA-256 的实物母版时禁止生图，禁止自行发明笑脸榴莲、绿色/米色纸箱或品牌图案。
   另执行完整性与实例门：挖空大坑、顶部剥开、开盆状露馅和手撕洞均不能冒充咬口；人物正在吃或产品仍在嘴边时，咬口只朝嘴部和人物大方向，嘴接触咬口侧，禁止破口朝镜头/观众或嘴接触完整背面。原片手持两颗/三颗必须等数替换，多颗共享约7厘米尺寸等级但各有轻微手工差异，叠放接触处只允许局部轻微压扁。多实例容器按稳定 ID 保持布局，拿起一颗后容器库存严格减一；包装和独立袋采用轻微错位、不同朝向和自然遮挡，禁止完美网格与等距全平行。
9. 主体替换后若人物、手、机位、背景或光线漂移，直接拒绝；若产品有两个以上核心错误，整张从原始干净图片重做，不回喂失败图。
10. 只有未启用身份替换、且确实只有一个局部主问题、其余产品和场景核心内容全部通过时，才允许一次单变量修改；同时换头与换产品的镜头永远不适用该豁免。仍失败或产生回归，停止该分支并回到原始图。
...
- `assets/q-scale-pressure-approved-test.png`：用户于 2026-08-19 指示加入知识库测试的尺寸与 Q 弹校准资产。只参考约 7 厘米产品相对同景深食指/掌心的比例、单指按压形成的连续宽圆碗状凹陷、平滑暖奶白完整皮面；不继承手部身份、背景、散粉、构图或拉开终点。它批准“静止尺寸与按压形变”作为测试基准，不等于整套五形态正式批准。
...
- `assets/individual-pouch-v1/front-user.png`、`side-user.png`：用户确认的单颗独立透明膜袋正面/侧面。必须按当前可见面单独绑定，只继承袋体、棕色内托、封边和可见版式；禁止继承实拍手部、房间背景、拍摄构图，也禁止把袋内食品图当作大福外观母版。批准记录和哈希见 `references/individual-pouch-v1.json`。
- `assets/aoqiaoxiaofang-retail-box-v1/`：用户确认的 15×15×4.5 厘米零售外盒四面实拍。按当前可见面选择，禁止用正面冒充侧面、用零售盒冒充独立袋或运输箱。

静态 V3 只附与当前唯一终点匹配的一张形态参考：`pressed` 用 01；`global_stretch` 可用 `user-stretch-sequence-01.png` 或 `02.png` 中一张，只借鉴整体受力方向与宽软形变；极小 `opening_window` 用 03，较大 `opening_window` 用 02。不得把多张形态参考同时附上。上述参考均不构成合格 `pre_break` 参考。

`opening_window_seed` 不附 `filling-neutral-light-reference.png`、`approved-opened-display-texture-v5.png`、`visual-mother-v5.png` 或任何大开口/两半/完整断面资产。小开口镜只使用当前原图、严格角色化的 `v3-user-reference-03-opening-seed.png` 与文字内馅合同；若参考图的大号扁平轮廓仍污染尺寸，停止附图并只用文字与当前同景深尺度锚点。

`shell-color-surface-clean.png` 只承担暖米白和平滑皮面基底，它本身不能证明已出现用户要求的可见细糯米粉。生成和 QA 仍必须单独要求：最外层具有侧向柔光下可见、无法逐粒辨认的极薄细糯米粉雾层；既不退化成完全无粉面团，也不变成可数白点或粗粉团。
...
- 包装可见时，四颗库存必须逐颗用稳定实例 ID 核算；每个实质可见实例分别记录轮廓家族、长宽比、直边占比、直角数和证据。任一颗变方或与手持/盘中几何身份不一致，整张拒收并从精确原图重做。
