...
def migrate(source: Path, output: Path | None = None) -> Path:
    ...
    packaging_rebind_shot_ids: list[str] = []
    for shot in shots.get("shots") or []:
        ...
        if state.get("packaging") not in (None, False, "none", "hidden"):
            ...
            state.pop("packaging_lock", None)
            packaging_rebind_shot_ids.append(str(shot.get("id")))
        else:
    ...
    project["migration_requirements"] = {
        "requires_manual_shot_map_rebuild": bool(ambiguous or topology_rebind_shot_ids or layout_rebind_shot_ids or packaging_rebind_shot_ids),
        ...
        "packaging_rebind_shot_ids": sorted(set(packaging_rebind_shot_ids)),
