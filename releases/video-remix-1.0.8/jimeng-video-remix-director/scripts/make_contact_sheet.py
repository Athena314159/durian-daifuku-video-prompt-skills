...
def make_sheets(
    ...
    def candidate_path(item: Dict[str, Any]) -> Path:
        if image_root is not None:
            return image_root.expanduser().resolve() / Path(item["frame"]).name
        return resolve(project_dir, item["frame"])

    first_path = candidate_path(candidates[0])
    ...
    output_dir = output_dir.expanduser().resolve() if output_dir else project_dir / "source" / "contact_sheets"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--image-root", type=Path, help="Optional directory containing normalized frames with the same basenames as the manifest frames.")
    parser.add_argument("--output-dir", type=Path, help="Optional output directory for the contact sheets.")
    return parser
