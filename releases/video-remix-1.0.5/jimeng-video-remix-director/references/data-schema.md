...
人物与产品同时替换时，`required_avatar_ids` 与 `required_product_ids` 都必须非空，并分别通过身份/授权审核和产品/状态审核。不得用一项通过代替另一项。美观/动作候选帧先通过结构与视觉 QA，仍只是视觉已审；总控完整展示按 unit 排序的单图总览并取得用户确认后，才能把 `approval_status` 改为 `user_approved`，并写入与图片 SHA-256 绑定的 `user_approval` 与全量 `gallery_receipt`。
