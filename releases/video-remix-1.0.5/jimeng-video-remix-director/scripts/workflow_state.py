...
PENDING_INPUTS = {"revised_script", "target_product_reference"}
...
def initialize(project_dir: Path, skill_dir: Path) -> dict[str, Any]:
    ...
    state.update(
        {
            ...
            "pending_inputs": state.get("pending_inputs") or [],
            "next_allowed_actions": state.get("next_allowed_actions")
            or ["import_source_video", "extract_source_transcript", "inventory_source_visuals"],
            "skill_versions": {
    ...
    state["status"] = derived_status(state)
...
def derived_status(state: dict[str, Any]) -> str:
    if state.get("current_stage") == "complete":
        return "complete"
    if state.get("blocked_by"):
        return "blocked"
    if state.get("source_ready") and state.get("current_stage") == "transcript_handoff":
        return "source_ready"
...
def main() -> int:
    ...
    sub.add_parser("mark-source-ready")
    await_parser = sub.add_parser("await-input")
    await_parser.add_argument("--input", required=True, choices=sorted(PENDING_INPUTS))
    resolve_input_parser = sub.add_parser("resolve-input")
    resolve_input_parser.add_argument("--input", required=True, choices=sorted(PENDING_INPUTS))
    ...
    with project_lock(project_dir):
        ...
        elif args.command == "mark-source-ready":
            state["source_ready"] = True
            if state.get("current_stage") == "intake":
                set_stage(state, "transcript_handoff", ["inline_editable_transcript", "accept_revised_script"])
            else:
                state["status"] = derived_status(state)
        elif args.command == "await-input":
            pending = list(state.get("pending_inputs") or [])
            if args.input not in pending:
                pending.append(args.input)
            state["pending_inputs"] = pending
            state["status"] = derived_status(state)
        elif args.command == "resolve-input":
            state["pending_inputs"] = [item for item in state.get("pending_inputs", []) if item != args.input]
            state["status"] = derived_status(state)
