...
from migrate_durian_daifuku_v2 import active_legacy_contamination, migrate  # noqa: E402
...
def main() -> int:
    with tempfile.TemporaryDirectory() as temporary:
        ...
        for token in ("约7厘米", "细糯米粉雾层", "连续果泥", "opening_window_seed", "到达终点后立即停止", "禁止顶部剥开", "原片手持两颗或三颗"):
        ...
        assert not active_legacy_contamination(migrated, "video-remix-1.0.10")
        ...
        assert migrated_v2_project["skill_release_lock"]["bundle_release_id"] == "video-remix-1.0.10"
        ...
        assert intake["skill_release"] == "video-remix-1.0.10"
        ...
        assert reuse["contract_binding"]["bundle_release_id"] == "video-remix-1.0.10"
        assert not active_legacy_contamination(migrated_v2, "video-remix-1.0.10")
        ...
        findings = active_legacy_contamination(migrated_v2, "video-remix-1.0.10")
